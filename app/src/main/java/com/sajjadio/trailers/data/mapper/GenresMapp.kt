...
internal fun mapToGenres(input: List<GenresDto>): List<Genres> {
    return input.map {
        Genres(
            id = it.id,
            name = it.name,
            imageUrl = getImageForGenre(it.name.toString())
        )
    }
}

private fun getImageForGenre(genreName: String): String {
